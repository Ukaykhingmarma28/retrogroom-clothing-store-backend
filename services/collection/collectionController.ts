...
export const getCollectionByCategory = async (req: Request, res: Response, next: NextFunction) => {
const { collectionCategory }  = req.params
  ...
  if (!collection) {
    return next(createHttpError(404, `No collection for ${collectionCategory} category`));
  }
